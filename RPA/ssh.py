    ...
    for i in ls: # 서버정보를 담은 리스트를 반복
        ...
        try:
            ...
            for i, cmd in enumerate(cmds):   # 명령어 리스트를 반복하여 수행
                ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command(cmd) # 표준콘솔입력(stdin), 표준콘솔출력(stdout), 표준에러출력(stderr) 리턴
                text = ''.join(ssh_stdout.readlines()) # 표준콘솔출력으로 전달된 stdout로부터 readlines()를 통해 출력 결과를 라인으로 읽어들이고 출력함
                log.info("커맨드 : {} 정보 : \n{}".format(cmd,text))
